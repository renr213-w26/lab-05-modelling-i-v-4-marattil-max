...
b.  Plotting the data is a useful first step because it helps us quickly visualize trends, identify strong associations, and develop research questions. Create an appropriate plot displaying the relationship between `weight` and `habit`. In 2-3 sentences, discuss the relationships observed.
    ...
    **\*\*\***
    ...
    -   **data for smokers shows a left skew**

    -   **all of the habit data has a great amount of outliers on the lower side of the wealth**

c.  Now, fit a linear model that predicts `weight` from `habit`. Provide the tidy summary output below.

    ```{r}
    #linear model
    linear_model<- lm (weight ~ habit, data = births14_habitgiven)

    #tidy summary
    tidy(linear_model, conf.int = FALSE, conf.level = 0.95)
    ```
...
d.  Write the estimated least squares regression line below using proper notation.
    ...
    ```{r}
    plot(0:10, main = expression(y ==7.270  - 0.593*x))
...
a.  Another researcher is interested in assessing the relationship between babies’ weights and mothers’ ages. Fit another linear model to investigate this relationship. Provide the summary output below.
    ...
    #linear model
    mage_model<- lm (weight ~ mage, data = births14_habitgiven)

    #tidy summary
    tidy(mage_model, conf.int = FALSE, conf.level = 0.95)
    ```

    ```{r}
    #add regression line
    ggplot(births14_habitgiven,aes(x= mage, y = weight)) +
      geom_point()+
...
b.  In 2-3 sentences, explain how the regression line to model these data is fit, i.e., based on what criteria R determines the regression line.
    ...
    **The fit of a regression line is based on minimizing the difference between an observed data point for birth weight and the predicted birth weight for the same x value( mother's age).**
...
c.  Interpret the intercept in the context of the data and the research question. Is the intercept meaningful in this context? Why or why not?
    ...
    **Intercept is the estimated birth weight, when mother's age is 0. While not logical in the realistic context, the intercept is an extrapolation to help anchor the line of best fit equation rather that be used by itself.**

d.  Interpret the slope in the context of the data and the research question.

    Slope is the value of increase in weight for every increase year of mother's age, showing by what factor of increase should the regression line increase. In this case, slope = 0.01498, shows that mother's age a very small but positive relationship with birth weight .
...
b.  Based on the goals of the analysis, what is the response variable?
    ...
    **the `parsim`: a measure of parasite similarity**
...
d.  Use the visualization to describe the relationship between the two variables.
    ...
    **There seems to be a strong negative relationship between divergence time and parasitism similarity between the 2 species. As time increase and the species evolve apart from one another in-terms of genetic and physical similarity, the measure of parasitic similarity has a decrease.**
...
a.  Fit the model and write the estimated regression equation.
    ...
    ```{r}
    #linear model
    parasim_model<- lm (parsim ~ divergence_time, data = parasites)

    #tidy summary
    tidy(parasim_model, conf.int = FALSE, conf.level = 0.95)
    ```
...
b.  Interpret the slope and the intercept in the context of the data.
    ...
    -   **Slope = -0.0065**
        ...
        **For each increase in million years, there is decrease in proportions of parasite similarity by 0.0065. This shows a weak negative relationship between divergence time and parasite similarity, therefore, increase difference between two species relates to decrease similarities in the parasites of the species themselves.**

    -   **Intercept = 0.3795**

        **At a point in time (0million years), where the two species was initally one species, the parasite similarity was 0.3795. This value is the maximum value for parasite similarity on average.**
...
d.  What do you notice about the prediction (regression) line that may be strange, particularly for very large divergence times?
    ...
    **The regression line visualization shows a predicted parsim value out of the 0-1 range and into the negative values as divergence time increases signifcantly.**
...
c.  Write a 1-2 sentence description of what you observe in the visualization.
    ...
    **After the transformation, the observed data is more linear and fiting with the regression line. Additionally, by adjusting the range, the predictive assuptions by the regression line makes more sense.**
...
a.  Report the slopes for each of these models. Use proper notation.
    ...
    **divergence_time:** $\Delta$**y = -0.0907**

    **distance:** $\Delta$**y = -0.0003**

    **BMdiff:** $\Delta$**y = -5.0444e-06**

    **precdiff:** $\Delta$**y = -0.0060**
...
b.  To answer the question of interest, would it be useful to compare the slopes in each model to choose the variable that is the strongest predictor of parasite similarity? Why or why not?
    ...
    **The domains of each variable pair are very different from one another (eg, divergence_time in million years vs distance in km). Thus, the values of the slopes alone are not comparable for any conclusions between the variables.**
...
b.  To answer our question of interest, would it be useful to compare the $R^2$ in each model to choose the variable that is the strongest predictor of parasite similarity? Why or why not?
    ...
    **Because** $R^2$ **is the coefficient of determination, it shows the proportion of variance in the specific variable comparison in a scale-free method. Through this, each value of \$R\^2\$ is isolated in its analysis but comparable in representing the fit of the model. Thus, divergence_time has the strongest predictor of parasite similarity, and BMdiff is the weakest predictor.** 
...
b.  **Visualization:** We are interested in learning more about GDP, and we’ll start with exploring the relationship between life expectancy and GDP. Create two visualizations:
    ...
    **There is a strong positive relationship between life expectancy and GDP per capita, which is better shown in the second model (using log), as it is a linearized version of the raw data and better fit for a linear regression application.**
...
c.  **Model fitting:**
    ...
    ```{r}
    #linear model
    life_expectancy_model<- lm (lifeExp ~ gdpPercap_log, data = gapminder_07)

    #tidy summary
    tidy(life_expectancy_model, conf.int = FALSE, conf.level = 0.95)
    ```
...
d.  **Model evaluation:**
    ...
    corr_r_squared <- cor(gapminder_07$lifeExp, gapminder_07$gdpPercap_log)^2
    corr_r_squared
    ```

    -   Interpret R-squared in the context of the data and the research question.

    **The R-squared value indicates that about 65 .4% of the variation in life expectancy across countries in 2007 can be explained by differences in GDP per capita. This suggests a strong positive association between the varibles. However, \~30% of the variation remains unexplained by the model, which could be attributed to other factors.**
...
**Yes, the trend appears different across continents based on the scatter plot. For example, Africa shows the widest spread with generally lower GDP and lower life expectancy. The slope appears flatter with many countries below the global trend. On the other hand, Europe & Oceania show the strongest positive relationship with consistently high life expectancy (75-82 years) across a range of GDP values**
...
a.  **Model fitting and interpretation:**
    ...
    ```{r}
    continent_model <- lm(lifeExp ~ gdpPercap_log + continent, data = gapminder_07)

    tidy(continent_model, conf.int = FALSE, conf.level = 0.95)
    ```
    ...
    **When GDP per capita is \$1 (log = 0), the model predicts a life expectancy of approximately \~20 years for African countries. However, this is not meaningful in practice because no country has a GDP per capita of \$1, and this represents extreme extrapolation outside the range of observed data.**
    ...
    **The slope for `gdpPercap_log` represents the expected change in life expectancy (in years) for a one-unit increase in log(GDP per capita)**

b.  **Prediction:** Predict the GDP of a country in Asia where the average life expectancy is 70 years old.
...
**The presentation transforms global development data through animated storytelling which maintains audience interest throughout the entire show. The moving bubble charts showed how countries evolved from their original state between two hundred years and helped understanding of complex statistical ideas through visual demonstrations. The most interesting part showed how China and Japan developed as they followed Western countries because both nations showed progress in health and wealth which disproved existing beliefs about worldwide inequality. The audience found it difficult to track country-specific movements during the first half of the 1800s because bubbles moved quickly but Rosling's physical demonstrations created an engaging and understandable experience. Global health and wealth have improved throughout history because the world has experienced unprecedented progress which proves people wrong who share pessimistic views about development. The storytelling elements together with energetic performance and data visualizations function as essential components for effective data communication which enables audience members to emotionally connect with data points.**
